...
class SmsForm extends AbstractForm
{
    /** @var array Форма */
    public array $form = [];
    ...
    {
        $this->entity_data = (new Sms())->find($this->entity_id);
    }
    ...
    public function form(int $id = 0): self
    {
        $this->form = [
            'id'          => $this->entity_id,
            'template_id' => (new Select())
                ->setLabel('Шаблон')
                ->setNameAndId('template_id')
                ->setItems(
                    static function () {
            ...
            'text'        => (new InputTextarea())
                ->setLabel('Текст')
                ->setValidationRule('required|max:420')
    ...
    {
        $result = [];
        foreach ($request->input('form') as $item) {
            if (! is_array($item)) {
                continue;
            }
